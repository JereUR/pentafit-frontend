...
const SelectedMembersActions: React.FC<Props> = ({
  selectedMembers,
  setSelectedMembers
}) => {
  ...
  const { deleteMembersById, addMembersToBusinesses, loadingTeamForm } =
    useTeam()
  ...
  const handleDelete = async () => {
    const res = await deleteMembersById(selectedMembers)
  ...
  const handleAdd = async () => {
    const res = await addMembersToBusinesses({
    ...
    <div className="flex gap-4">
      <div>
        <div className="relative">
          ...
          >
            ({selectedMembers.length}) Agregar a
            ...
            <div className="flex flex-col gap-4 justify-center items-center bg-background border border-primary-orange-600 p-8 rounded-lg shadow-md max-w-xl">
              <p>
                {selectedMembers.length === 1
                  ? `Agregar el usuario seleccionado a los siguientes negocios: `
                  : `Agregar los ${selectedMembers.length} usuarios seleccionados a los siguientes negocios: `}
              </p>
      ...
      <div>
        ...
        >
          ({selectedMembers.length}) <FaTrash />
        </Button>
            ...
            <div className="flex flex-col gap-4 justify-center items-center bg-background border border-primary-orange-600 p-8 rounded-lg shadow-md">
              <p>
                {selectedMembers.length === 1
                  ...
                  : `¿Está seguro de que desea eliminar lo ${selectedMembers.length} usuarios seleccionados?`}
              </p>
              <div className="flex justify-end gap-2">
                <Button
                  variant="secondary"
...
export default SelectedMembersActions
